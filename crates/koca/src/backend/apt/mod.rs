...
pub fn install_plan(packages: &[String]) -> Result<(ResultPayload, Vec<String>), ProtocolError> {
    ...
    if !output.status.success() {
        ...
        return Err(ProtocolError {
            code: classify_apt_error(&stderr),
    ...
    let actions: Vec<PlannedAction> = inst_lines
        .into_iter()
        .map(|line| {
            ...
            PlannedAction {
                ...
                action: if line.old_version.is_some() {
                    ActionKind::Upgrade
                } else {
                    ActionKind::Install
                },
...
fn run_apt_with_status(
    ...
    let child = std::process::Command::new("apt-get")
        ...
        .map_err(|e| ProtocolError {
            code: ErrorCode::Internal,
            message: format!("failed to spawn apt-get: {e}"),
        })?;
    drop(status_write);
    ...
    let output = child.wait_with_output().map_err(|e| ProtocolError {
        code: ErrorCode::Internal,
        message: format!("failed to wait for apt-get: {e}"),
    })?;
    ...
    let join_handle = tokio::spawn(async move {
        if is_remove {
            ...
            return result.unwrap_or_else(|_| {
                Err(ProtocolError {
                    code: ErrorCode::Internal,
                    message: "remove task panicked".into(),
                })
            });
        }
        let items = tokio::task::spawn_blocking({
            let pkgs = pkgs.clone();
            move || get_download_items(&pkgs)
        })
        .await
        .unwrap_or_else(|_| {
            Err(ProtocolError {
        ...
        result.unwrap_or_else(|_| {
            Err(ProtocolError {
                code: ErrorCode::Internal,
                message: "install task panicked".into(),
            })
        })
    });
