...
fn apt_package_for_filename(filename: &str, packages: &[String]) -> String {
    filename
        ...
        .unwrap_or_else(|| {
            packages
                .iter()
                .find(|pkg| filename.starts_with(&format!("{pkg}_")))
                .cloned()
...
/// unit tests.
fn parse_print_uris_line(
    line: &str,
    packages: &[String],
) -> Result<Option<DownloadItem>, ProtocolError> {
    let line = line.trim();
    if !line.starts_with('\'') {
        return Ok(None);
    }
    let end = line[1..].find('\'').ok_or_else(|| ProtocolError {
        code: ErrorCode::Internal,
        message: format!("invalid --print-uris line: {line}"),
    })? + 1;
    ...
    let filename = parts
        ...
        .ok_or_else(|| ProtocolError {
            code: ErrorCode::Internal,
            message: format!("missing filename in --print-uris line: {line}"),
        })?
        .to_string();
    let size = parts
        .next()
        .ok_or_else(|| ProtocolError {
            code: ErrorCode::Internal,
            message: format!("missing size in --print-uris line: {line}"),
        })?
        .parse()
        .map_err(|_| ProtocolError {
            code: ErrorCode::Internal,
            message: format!("invalid size in --print-uris line: {line}"),
        })?;
    let (hash_kind, hash_value) = match parts.next() {
        Some(hash) => {
            let (hash_name, hash_value) = hash.split_once(':').ok_or_else(|| ProtocolError {
                code: ErrorCode::Internal,
                message: format!("invalid hash in --print-uris line: {line}"),
            })?;
            (
                parse_hash_kind(hash_name).ok_or_else(|| ProtocolError {
                    code: ErrorCode::Internal,
                    message: format!("unsupported hash type from apt: {hash_name}"),
                })?,
    ...
    let mirror_key = reqwest::Url::parse(&url)
        .ok()
        .and_then(|u| {
            u.host_str()
...
fn run_apt_cache_query(args: &[&str]) -> Result<std::process::Output, ProtocolError> {
    std::process::Command::new("apt-cache")
        ...
        .map_err(|e| ProtocolError {
            code: ErrorCode::Internal,
            message: format!("failed to run apt-cache: {e}"),
        })
}

fn query_archive_info(
    packages: &[String],
) -> Result<HashMap<String, AptArchiveInfo>, ProtocolError> {
    ...
    for line in String::from_utf8_lossy(&output.stdout).lines() {
        if line.is_empty() {
            ...
            if let Some(info) = current.as_mut() {
                info.sha256 = val.trim().to_string();
            }
        }
    }
    if let Some(info) = current.take() {
        ...
        for item in &mut items {
            if item.hash_value.is_empty() {
                if let Some(info) = fallback.get(&item.filename) {
                    ...
                    return Err(ProtocolError {
                        code: ErrorCode::Internal,
                        message: format!("missing fallback metadata for {}", item.filename),
                    });
                }
            }
        }
...
fn ensure_cache_dirs() -> Result<(), ProtocolError> {
    fs::create_dir_all(ARCHIVES_DIR).map_err(|e| ProtocolError {
        code: ErrorCode::Internal,
        message: format!("create {}: {e}", ARCHIVES_DIR),
    })?;
    fs::create_dir_all(PARTIAL_DIR).map_err(|e| ProtocolError {
        code: ErrorCode::Internal,
        message: format!("create {}: {e}", PARTIAL_DIR),
    })?;
    Ok(())
}

fn compute_hash(path: &Path, kind: HashKind) -> Result<String, ProtocolError> {
    let mut file = File::open(path).map_err(|e| ProtocolError {
        code: ErrorCode::Internal,
    ...
    loop {
        let read = file.read(&mut buf).map_err(|e| ProtocolError {
            code: ErrorCode::Internal,
    ...
    Ok(if let Some(hasher) = md5 {
        format!("{:x}", hasher.finalize())
    } else if let Some(hasher) = sha1 {
        format!("{:x}", hasher.finalize())
    } else {
        format!("{:x}", sha256.expect("sha256 hasher").finalize())
    })
}
...
async fn download_one(
    ...
    if item.url.starts_with("file:") {
        let url = reqwest::Url::parse(&item.url).map_err(|e| ProtocolError {
            code: ErrorCode::Internal,
            message: format!("invalid file URL {}: {e}", item.url),
        })?;
        let src = url.to_file_path().map_err(|_| ProtocolError {
            code: ErrorCode::Internal,
            message: format!("cannot convert file URL to path: {}", item.url),
        })?;
        fs::copy(&src, &item.temp_path).map_err(|e| ProtocolError {
            code: ErrorCode::Internal,
            message: format!(
                "copy {} -> {}: {e}",
                src.display(),
        ...
        let mut response = client
            ...
            .map_err(|e| ProtocolError {
                code: ErrorCode::Internal,
                message: format!("download failed for {}: {e}", item.filename),
            })?;
        if !response.status().is_success() {
            return Err(ProtocolError {
                code: ErrorCode::Internal,
                message: format!("HTTP {} for {}", response.status(), item.url),
            });
        }
        let mut file = File::create(&item.temp_path).map_err(|e| ProtocolError {
            code: ErrorCode::Internal,
            message: format!("create {}: {e}", item.temp_path.display()),
        })?;
        let mut last_emit = Instant::now();
        while let Some(chunk) = response.chunk().await.map_err(|e| ProtocolError {
            code: ErrorCode::Internal,
            message: format!("download {}: {e}", item.filename),
        })? {
            file.write_all(&chunk).map_err(|e| ProtocolError {
                code: ErrorCode::Internal,
                message: format!("write {}: {e}", item.temp_path.display()),
            })?;
    ...
    if !verify_download(&item.temp_path, item)? {
        ...
        return Err(ProtocolError {
            code: ErrorCode::Internal,
            message: format!("verification failed for {}", item.filename),
        });
    }
    fs::rename(&item.temp_path, &item.final_path).map_err(|e| ProtocolError {
        code: ErrorCode::Internal,
        message: format!(
            "move {} -> {}: {e}",
            item.temp_path.display(),
...
pub(crate) async fn download_packages(
    ...
    for item in items {
        if verify_download(&item.final_path, item)? {
            cached.push(item.clone());
        } else {
            let _ = fs::remove_file(&item.final_path);
            let _ = fs::remove_file(&item.temp_path);
    ...
    let semaphores: Arc<HashMap<String, Arc<Semaphore>>> = Arc::new(
        needed
            .iter()
            .map(|item| {
                (
                    item.mirror_key.clone(),
                    Arc::new(Semaphore::new(PER_MIRROR_CONCURRENCY)),
                )
            })
            .collect(),
    );
    ...
    for item in needed {
        ...
        handles.push(tokio::spawn(async move {
            let permit = semaphores
                .get(&item.mirror_key)
                .expect("mirror semaphore")
                .clone()
                .acquire_owned()
                .await
                .map_err(|_| ProtocolError {
                    code: ErrorCode::Internal,
                    message: "download semaphore closed".into(),
                })?;
            {
                let mut state = state.lock().unwrap();
                add_active(&mut state.active, &item.package);
            }
            emit_progress(&tx, &state, total_bytes);
            let result = download_one(&client, &item, total_bytes, &state, &tx).await;
            {
                let mut state = state.lock().unwrap();
                remove_active(&mut state.active, &item.package);
            }
            drop(permit);
            emit_progress(&tx, &state, total_bytes);
            result?;
